...
function riseset(date, latitude, longitude){
...
function A(date, latitude, longitude){
	...
	M_radians = M / 180 * Math.PI;
	C = 1.9148*Math.sin(M_radians) + 0.0200*Math.sin(2*M_radians) + 0.0003*Math.sin(3*M_radians);
	L = (M+C+180+102.9372)%360;
	J_transit = 2451545.5 + J_star + 0.0053*Math.sin(M_radians) - 0.0069*Math.sin(2*L);
	sind = Math.sin(L/180*Math.PI)*Math.sin(23.44/180*Math.PI);
	cosw = ( Math.sin(-0.83/180*Math.PI) - Math.sin(latitude/180*Math.PI)*sind ) / (Math.cos(latitude/180*Math.PI)*Math.cos(Math.asin(sind)));
	w = Math.acos(cosw)*180/Math.PI;

	
	var J_set = J_transit + w/360;

	return (J_rise - J_set)*MS_PER_DAY;
}
...
function latitudeSearch(date, duration, longitude=0, div=null){
	...
	while(southern < northern){
		...
		if(a != undefined && Math.abs(a - duration) < DESIRED_EPSILON)
			return pot;
